...
## Topic 2: Intertemporal Choice --- Making Choices Over Time
...
Let's understand the standard model and (quickly) do some example problems. (This is a great time to ask questions.)
...
- After 1 year, you'll have $(\$1000)\ast (1.10)=\$1100$.

- After 2 years, you'll have $(\$1100)\ast (1.10)=\$1210$.

- After 3 years, you'll have $(\$1210)\ast (1.10)=\$1331$.
...
\underline{Definition}: The \emph{period-}$t$\emph{\ interest rate} $r_{t}$
is the interest rate between period $t$ and period $t+1$. In other words, if
in period $t$ your principal is $P_{t}$, then in period $t+1$ it becomes $%
P_{t+1}=(1+r_{t})P_{t}$.

\vspace*{.1in}

Hence, if $P_{t}$ is the principal in your bank account in period $t$, and
if your bank account pays per-period interest rates $(r_{t},r_{t+1},...)$,
then:

\begin{itemize}
\item $P_{t+1}=(1+r_{t})P_{t}$.

\item $P_{t+2}=(1+r_{t+1})P_{t+1}=(1+r_{t+1})(1+r_{t})P_{t}$.

\item $P_{t+3}=(1+r_{t+2})P_{t+2}=(1+r_{t+2})(1+r_{t+1})(1+r_{t})P_{t}$.
...
\[
PDV=\frac{R_{t+1}}{(1+r_{t})}+\frac{R_{t+2}}{(1+r_{t})(1+r_{t+1})}+\frac{%
R_{t+3}}{(1+r_{t})(1+r_{t+1})(1+r_{t+2})}.
\]
